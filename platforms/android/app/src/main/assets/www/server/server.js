...
/*
    Return the logs
*/
app.get('/search/:dronelogs', function (req, res) {
    if (!dbObject) {
        return res.status(500).end();
    }

    var drone = (req.params && req.params.dronelogs) ? req.params.dronelogs : null;
    if (!drone) {
        return res.status(400).end();
    }

    console.log(`Search logs :: ${drone}`);
...
/*
    Save the logs
*/
app.post('/DroneLogs/:dronelogs/dronelogs', function(req, res) {
    if (!dbObject) {
        return res.status(500).end();
    }
